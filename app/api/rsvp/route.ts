...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Validate attendance is explicitly set (not null/undefined)
    if (body.attending === null || body.attending === undefined) {
      return NextResponse.json(
        { error: "Please select whether you will be attending (Yes or No)" },
        { status: 400 }
      );
    }
    ...
    const rsvp: RSVP = {
      ...
      plus_one_names:
        body.plus_one_names &&
        Array.isArray(body.plus_one_names) &&
        body.plus_one_names.length > 0
          ? body.plus_one_names.filter(
