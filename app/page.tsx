...
function ContentRenderer({ 
  ...
  switch (displayLocation) {
    case 'London':
      console.log('[ContentRenderer] Rendering LondonContent with guest:', { name: guest.name, allowed_plus_ones: guest.allowed_plus_ones });
      return <LondonContent inviteCode={code} guest={guest} />;
    case 'Portugal':
      console.log('[ContentRenderer] Rendering PortugalContent with guest:', { name: guest.name, allowed_plus_ones: guest.allowed_plus_ones });
      return <PortugalContent inviteCode={code} guest={guest} />;
    case 'Lagos':
    default:
      console.log('[ContentRenderer] Rendering LagosContent with guest:', { name: guest.name, allowed_plus_ones: guest.allowed_plus_ones });
  ...
  return (
    <main className="min-h-screen">
      ...
      <nav className="border-b border-gray-200 py-4 px-4">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex gap-6 text-sm uppercase tracking-wider">
            ...
            <a href="/#travel" className="hover:underline">TRAVEL & STAY</a>
            <a href="/#registry" className="hover:underline">REGISTRY</a>
          </div>
          <h1 className="text-2xl md:text-3xl font-serif">DAMOLA & JOÃO</h1>
          <div className="flex gap-6 items-center">
            <a href="/#faqs" className="text-sm uppercase tracking-wider hover:underline">FAQS</a>
            <a href="/#rsvp" className="text-sm uppercase tracking-wider border border-black px-4 py-2 hover:bg-black hover:text-white transition-colors">RSVP</a>
          </div>
        </div>
      </nav>
      ...
      {(!code || !guest) && code !== undefined && (
        <section className="py-16 px-4 bg-gray-50">
          <div className="max-w-2xl mx-auto text-center">
            <h2 className="text-2xl md:text-3xl font-serif mb-8">ENTER YOUR INVITE CODE</h2>
            <CodeInput />
            {code && !guest && (
              <p className="mt-4 text-sm text-red-600">
                Invalid code. Please check and try again, or continue to view Lagos information.
              </p>
            )}
          </div>
        </section>
      ...
      <Suspense fallback={<div className="min-h-screen flex items-center justify-center">Loading...</div>}>
        <ContentRenderer code={code} locationParam={locationParam} guest={guest} />
      </Suspense>
    </main>
  );
}
