...
export default function LocationNav({ currentLocation, allowedLocations, code }: LocationNavProps) {
  // Only show navigation if user has access to multiple locations
  if (allowedLocations.length <= 1) {
    return null;
  }

  const otherLocations = allowedLocations.filter(loc => loc !== currentLocation);
  ...
  return (
    <div className="py-8 text-center border-t border-b border-gray-300">
      <p className="text-sm uppercase tracking-wider mb-4 text-gray-600">Other Locations</p>
      <div className="flex justify-center gap-6 flex-wrap">
        {otherLocations.map(location => {
          const href = code ? `/?code=${code}&location=${location}` : `/?location=${location}`;
          return (
            <Link
              key={location}
              href={href}
